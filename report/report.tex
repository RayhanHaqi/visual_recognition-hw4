...
%, Document Identity, \title{\textbf{Visual Recognition using Deep Learning}\\ \large Homework 4: Image Restoration}
\author{
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{promptir_architecture_hires.pdf}
...
\begin{itemize}
    ...
    \item \textbf{Batch Size:} 1 to 3 (full $256 \times 256$ images). Bfloat16 mixed precision enables BS=3 without OOM.
...
\caption{Training and validation curves for Run 8 (L1 loss, PSNR-monitored) and Run 9 (L1+MSE, $\lambda=0.05$). Left: validation PSNR plateaus around epoch 80 to 100 for both runs, with Run 9 achieving a slightly higher best val\_psnr (30.749 vs 30.718). Right: training and validation loss curves converge without overfitting; Run 9's training loss tracks slightly below Run 8's.}
...
\begin{tabularx}{\textwidth}{@{}>{\raggedleft\arraybackslash}p{0.55cm} >{\raggedright\arraybackslash}X >{\centering\arraybackslash}p{0.6cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{2.35cm}@{}}
\toprule
...
15 & derain specialist, p256, bs=4 & 149 & val\_psnr\_derain=28.82; train only & N/A \\
16 & desnow specialist, p256, bs=4 (5090) & 112 & val\_psnr\_desnow=31.35; salvaged avg5 TTA & N/A \\
...
\textbf{Ens.} & \textbf{R9 + R8 + R11 avg5 + R11 TTA ensemble} & \textbf{N/A} & \textbf{prediction ensemble (equal weight)} & \textbf{31.84 to 31.85} \\ \bottomrule
\end{tabularx}
\end{table}

\subsection{Key Findings}

\subsubsection{Patch Size Optimization (Runs 1 to 8)}
...
\textbf{Conclusion:} Strategy~C is valuable as a negative result: task specialists improve per-task validation but do not transfer through pseudo-target distillation to a better \emph{single} model or beat prediction ensembling. The final submission remains the Run~9-based prediction ensemble at 31.84 to 31.85 PSNR.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Run} & \textbf{Local val\_psnr} & \textbf{Public TTA PSNR} & \textbf{Val to Public Gap} \\ \midrule
...
Runs 9 to 11 provide per-task validation metrics (introduced as a measurement-first modification). Table \ref{tab:pertask} shows the per-task breakdown from Run 9.
...
\begin{itemize}
    \item \textbf{Stage 2 Merged Retraining:} Training from scratch on Train+Val merged data with no validation monitor degraded PSNR from 30.5 to 21.7 (Runs 1 to 3). The loss of a validation checkpoint selection mechanism makes this approach fundamentally unreliable on small datasets.
    ...
    \item \textbf{Strategy C (Specialists + Distillation):} Task-only specialists (Runs 15 to 16) reached strong validation on their respective degradations but Run~18 distillation scored only $\sim$26.3 public PSNR, far below the 31.84 ensemble. Do not use for final submission.
    \item \textbf{RTX 5090 / Dual-GPU Training:} Blackwell GPU training caused recurring CUDA launch failures and post-reboot SSH instability; all reliable results used RTX~4090 only (\texttt{DISABLE\_CUDNN=1} on 50-series when required).
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Prediction Ensembling:} Equal-weight pixel averaging of 4 complementary checkpoints achieved 31.84 to 31.85, outperforming all single-model approaches by +0.07 dB and establishing the final submission.
...
Prediction ensembling at 31.84 to 31.85 is the strongest result. Strategy~C showed that specialist models and pseudo-target distillation are \emph{not} recommended on this dataset: local validation gains do not transfer to the hidden test set. Further improvements, if pursued, should focus on additional diverse full all-in-one models for ensembling rather than task-only training or distillation. Weighted ensembles (e.g., 60\% Run~9) may yield marginal 0.02 to 0.05 dB gains.

The submission with the best public PSNR is \path{submission/ensemble-diverse.zip} (or equivalent weighted variants at 31.85), prediction ensemble of Run~9 TTA, Run~8 TTA, Run~11 avg5 TTA, and Run~11 TTA. The single-model best is \path{submission/stage1-p256-tta-run9-l1mse.zip} at 31.77. \textbf{Do not submit} Run~18 distillation checkpoints (public PSNR $\sim$26.3).
