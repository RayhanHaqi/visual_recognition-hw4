...
Table \ref{tab:results} summarizes all experiments and their CodaBench public PSNR scores. Starting from an initial baseline of 30.52, systematic improvements raised the best score to 31.84 — a gain of 1.32 dB.
...
\textbf{Ens.} & \textbf{R9 + R8 + R11 avg5 + R11 TTA ensemble} & \textbf{---} & \textbf{prediction ensemble (equal weight)} & \textbf{31.84} \\ \bottomrule
\end{tabular}
\end{table}
...
This project successfully trained a PromptIR model from scratch for all-in-one rain and snow image restoration. Starting from a baseline of 30.52 PSNR, systematic experimentation yielded a final best score of \textbf{31.84 PSNR} via prediction ensembling of four strong single-model checkpoints, representing a 1.32 dB improvement. The single-model best is 31.77 (Run 9).
...
\begin{enumerate}
    ...
    \item \textbf{Prediction Ensembling:} Equal-weight pixel averaging of 4 complementary checkpoints achieved 31.84, outperforming all single-model approaches by +0.07 dB and establishing the final submission.
\end{enumerate}
...
Prediction ensembling at 31.84 is the strongest result, and further improvements are more likely to come from additional strong single-model checkpoints with diverse training recipes than from parameter-level ensembling or single-model experimentation. Promising unexplored directions include:
\begin{itemize}
    \item \textbf{Weighted Prediction Ensembles:} The equal-weight diverse ensemble already outperforms all singles. Optimizing ensemble weights (e.g., Run 9 at 60\%) or dropping the weakest contributor (Run 11 TTA at 31.72) may yield additional 0.02--0.05 dB.
    \item \textbf{Derain Oversampling:} While rain loss weighting failed because it changed the optimization landscape, pure data oversampling (duplicating rain images without changing the loss) was not independently evaluated and remains a plausible direction.
    \item \textbf{Charbonnier Loss:} The robust pixel-loss variant commonly used in BasicSR-style restoration is untested and may improve derain where L1 struggles with outlier pixels.
    \item \textbf{Longer Training:} 200+ epochs with cosine restarts, now feasible given the clear ensemble benchmark, may extract marginal gains.
\end{itemize}

The submission with the best public PSNR is \texttt{submission/ensemble-diverse.zip} at 31.84 (prediction ensemble of Run 9 TTA + Run 8 TTA + Run 11 avg5 TTA + Run 11 TTA). The single-model best is \texttt{submission/stage1-p256-tta-run9-l1mse.zip} at 31.77.
