...
13 & p256, bf16, bs=3, L1+MSE ($\lambda$=0.025), grad-ckpt & -- & pending submission & -- \\ \bottomrule
\end{tabular}
\end{table}
...
\subsubsection{Memory and Speed Optimizations for Larger Batch Sizes (Run 13)}
Run 13 introduces a comprehensive memory optimization pipeline targeting a physical batch size of 3 at $256 \times 256$ resolution ($\sim$20--22 GB VRAM) to increase training data diversity per optimization step. Four instrumentation changes were implemented, summarized in Table \ref{tab:opt_flags}.

\begin{table}[htbp]
\centering
\caption{Memory and speed optimization flags added for Run 13.}
\label{tab:opt_flags}
\small
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Flag} & \textbf{Effect} & \textbf{Memory Impact} \\ \midrule
\texttt{--precision bf16-mixed} & Automatic mixed precision with bfloat16 & $\sim$2$\times$ reduction vs fp32 \\
\texttt{--gradient\_checkpointing highres} & Activation checkpointing on 12 full-resolution blocks & $\sim$40\% activation reduction \\
\texttt{--compile} & \texttt{torch.compile} graph optimization & Negligible, but +10--20\% throughput \\
\texttt{cudnn.benchmark=True} & Fast kernel autotuning for fixed shapes & None, speed only \\ \bottomrule
\end{tabular}
\end{table}

\textbf{Bfloat16 mixed precision} is the primary enabler: the 4090 GPU (sm\_89) supports full bfloat16 tensor cores, halving activation memory and increasing arithmetic throughput compared to the previous FP32 configuration. Since PromptIR's L2-normalized attention and GELU activations are numerically stable in bfloat16, precision-related degradation is not expected.

\textbf{Gradient checkpointing} targets only the 12 transformer blocks operating at full $256 \times 256$ resolution (encoder level 1, decoder level 1, and refinement blocks), where activations dominate VRAM. These blocks are wrapped with PyTorch's \texttt{torch.utils.checkpoint.checkpoint\_sequential} with the recommended \texttt{use\_reentrant=False} mode, recomputing intermediate activations during the backward pass instead of storing them. The remaining 35 blocks at lower spatial resolutions are left unmodified to avoid unnecessary compute overhead.

A critical implementation detail is state dict compatibility: checkpointing wraps \texttt{nn.Sequential} blocks with a \texttt{CheckpointedSequential} wrapper that adds a \texttt{blocks.} prefix to parameter keys. The inference pipeline includes a \texttt{\_strip\_checkpoint\_wrapper} utility that normalizes these keys back to the original PromptIR format before \texttt{load\_state\_dict}, ensuring that inference checkpoints can be loaded with or without gradient checkpointing enabled.

\textbf{Run 13 configuration:} $P=256$, $B=3$, bf16-mixed, L1+MSE ($\lambda=0.025$), PSNR-monitored checkpointing, 150 epochs. The reduced $\lambda=0.025$ tests whether lower MSE weighting (below the Run 9 optimum of 0.05) provides additional benefit. Activation checkpointing is held as a fallback (\texttt{GRADIENT\_CHECKPOINTING=highres}) if BS=3 encounters OOM, while \texttt{torch.compile} is available as an opt-in throughput booster. The CUDA allocator is configured with \texttt{PYTORCH\_ALLOC\_CONF=expandable\_segments:True} to reduce memory fragmentation from the 47-transformer-block architecture.
...
12 (L1+MSE $\lambda$=0.10)   & 30.7149 & 31.72 & 1.01 \\ \bottomrule
\end{tabular}
\end{table}
...
The Run 13 experiment (ongoing) represents the most promising remaining direction: bfloat16 mixed precision enables batch size 3 without architectural changes, and the reduced MSE weight $\lambda=0.025$ probes whether $\lambda=0.05$ was overshooting the L1-MSE balance. Additional unexplored directions include:
\begin{itemize}
    \item Rain-specific data augmentation (rain-streak augmentation, intensity variation, heavier dropout for rain images).
    \item Longer training (200+ epochs) with cosine restarts, now feasible with the faster bf16 training pipeline.
    \item Derain loss weighting: the 2.2 dB gap between derain and desnow (Table~\ref{tab:pertask}) suggests that up-weighting rain image losses could close the bottleneck without architectural modifications.
    \item \texttt{torch.compile} for throughput: if the Run 13 pipeline is stable, enabling \texttt{--compile} could reduce training time by 10--20\%, enabling more experiments within the deadline constraint.
\end{itemize}

These directions have not been tested; all are contingent on Run 13 results and remaining compute budget.

The submission with the best public PSNR is \texttt{submission/stage1-p256-tta-run9-l1mse.zip} at 31.77. An additional experiment (Run 13) is in progress as of this writing, testing bfloat16 mixed-precision training at batch size 3 with L1+MSE ($\lambda=0.025$); results will be appended to the competition leaderboard before the deadline.
